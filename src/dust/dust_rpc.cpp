...
Value getbalance_MP(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            ...
            "\nReturns the Dust Protocol balance for a given address and currency/property.\n"
            "\nResult:\n"
            "n    (numeric) The applicable balance for address:currency/propertyID pair\n"
            "\nExamples:\n"
            ">dustd getbalance_MP 1EXoDusjGwvnjZUyKkxZ4UHEf77z6A5S4P 1\n"
        );
...
if (fHelp || params.size() < 4 || params.size() > 6)
        throw runtime_error(
            ...
            "\nExamples:\n"
            ">dustd send_MP 1FromAddress 1ToAddress PropertyID Amount 1RedeemAddress\n"
        );
...
if (fHelp || params.size() < 3 || params.size() > 4)
        throw runtime_error(
            ...
            "txid    (string) The transaction ID of the sent transaction\n"
            "\nExamples:\n"
            ">dustd send_MP 1FromAddress PropertyID Amount 1RedeemAddress\n"
        );
...
if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            ...
            "\nCreates and broadcasts a raw Dust protocol transaction.\n"
            "\nParameters:\n"
            "FromAddress   : the address to send from\n"
            ...
            "txid    (string) The transaction ID of the sent transaction\n"
            "\nExamples:\n"
            ">dustd sendrawtx_MP 1FromAddress <tx bytes hex> 1ToAddress 1RedeemAddress\n"
        );
...
Value trade_MP(const Array& params, bool fHelp)
{
    ...
    if (action <= 0 || CDustTransaction::CANCEL_EVERYTHING < action)
    ...
    if (action <= CDustTransaction::CANCEL_ALL_FOR_PAIR)
    {
    ...
    if (action <= CDustTransaction::CANCEL_AT_PRICE)
    {
        CMPSPInfo::Entry sp;
...
Value listblocktransactions_MP(const Array& params, bool fHelp)
{
   if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nReturns all Dust Protocol transactions in a block.\n"
            "\nArguments:\n"
...
int populateRPCTransactionObject(uint256 txid, Object *txobj, string filterAddress = "")
{
    ...
    if (p_txlistdb->exists(wtxid))
    {
        ...
        if (0 <= parseRC) //negative RC means no MP content/badly encoded TX, we shouldn't see this if TX in levelDB but check for sanity
        {
            ...
            else
            {
                ...
                {
                    return -3336; // "Not a Dust Protocol transaction"
                }
    ...
    {
        return MP_TX_IS_NOT_DUST_PROTOCOL;
    }
...
Value gettransaction_MP(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nGet detailed information about a Dust Protocol transaction <txid>\n"
            "\nArguments:\n"
    ...
    if (0>populateResult)
    {
        ...
        switch (populateResult)
        {
            ...
            case MP_TX_IS_NOT_DUST_PROTOCOL:
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Not a Dust Protocol transaction");
        }
    }
...
Value getinfo_MP(const Array& params, bool fHelp)
{
    ...
    // provide the dust and bitcoin version and if available commit id
    infoResponse.push_back(Pair("dustversion", DustVersion()));
    infoResponse.push_back(Pair("bitsendcoreversion", BitsendCoreVersion()));
    ...
    string global_alert_message = getDustAlertString();
    int32_t alertType = 0;
...
Value gettrade_MP(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "gettrade_MP \"txid\"\n"
            "\nGet detailed information and trade matches for a Dust Protocol MetaDEx trade offer <txid>\n"
            "\nArguments:\n"
            "1. \"txid\"    (string, required) The transaction id\n"
            "\nResult:\n"
            "{\n"

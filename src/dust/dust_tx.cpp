// Dust Protocol transaction code
...
int CDustTransaction::step1()
{
  ...
  file_log("\t            type: %u (%s)\n", type, c_strDustProtocolTXType(type));

  return (type);
}

// extract alert info for alert packets
int CDustTransaction::step2_Alert(std::string *new_global_alert_message)
{
  ...
  if (
     // TESTNET
     (sender == "test1") || // 
     (sender == "TestAddress") || //TODO: change after
     // MAINNET
     (sender == "iIamJoShafeSt") || // 
     (sender == "iSatoshi") || 
     (sender=="iAnyone2Else3Who4Should5Be6Here") // 
     ) authorized = true;
...
int CDustTransaction::step2_Value()
{
...
bool CDustTransaction::isOverrun(const char *p, unsigned int line)
{
...
const char *CDustTransaction::step2_SmartProperty(int &error_code)
{
...
unsigned int prop_id;
  ...
  if ((DUST_PROPERTY_MSC != ecosystem) && (DUST_PROPERTY_TMSC != ecosystem))
  {
...
int CDustTransaction::step3_sp_fixed(const char *p)
{
...
int CDustTransaction::step3_sp_variable(const char *p)
{
  if (!p) return (PKT_ERROR_SP -1);
...
static const char * const subaction_name[] = { "empty", "new", "update", "cancel" };
      ...
      if ((DUST_PROPERTY_TMSC != property) && (DUST_PROPERTY_MSC != property))
      {
...
int CDustTransaction::logicMath_AcceptOffer_BTC()
{
...
int CDustTransaction::logicMath_MetaDEx(CMPMetaDEx *mdex_o)
{
...
int CDustTransaction::logicMath_GrantTokens()
{
...
int CDustTransaction::logicMath_RevokeTokens()
{
    int rc = PKT_ERROR_TOKENS - 1000;
...
int CDustTransaction::logicMath_ChangeIssuer()
{
  int rc = PKT_ERROR_TOKENS - 1000;
...
int CDustTransaction::logicMath_SavingsMark()
{
int rc = -12345;
...
int CDustTransaction::logicMath_SavingsCompromised()
{
int rc = -23456;
...
char *dust::c_strDustProtocolTXType(int i)
{
  switch (i)
  {
    ...
    case DUST_MESSAGE_TYPE_ALERT: return ((char *)"ALERT");

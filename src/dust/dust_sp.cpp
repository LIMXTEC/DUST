...
unsigned int CMPSPInfo::updateSP(unsigned int propertyID, Entry const &info)
{
    ...
    if (DUST_PROPERTY_MSC == propertyID || DUST_PROPERTY_TMSC == propertyID) {
      return propertyID;
    }
...
unsigned int CMPSPInfo::putSP(unsigned char ecosystem, Entry const &info)
{
    ...
    unsigned int res = 0;
    switch(ecosystem) {
    case DUST_PROPERTY_MSC: // dust ecosystem, MSC: 1, TMSC: 2, First available SP = 3
      res = next_spid++;
      break;
    case DUST_PROPERTY_TMSC: // Test MSC ecosystem, same as above with high bit set
      res = next_test_spid++;
      break;
...
bool CMPSPInfo::getSP(unsigned int spid, Entry &info)
{
    ...
    if (DUST_PROPERTY_MSC == spid) {
      info = implied_msc;
      return true;
    } else if (DUST_PROPERTY_TMSC == spid) {
      info = implied_tmsc;
      return true;
    }
...
{
    // special cases for constant SPs MSC and TMSC
    if (DUST_PROPERTY_MSC == spid || DUST_PROPERTY_TMSC == spid) {
      return true;
    }
...
CMPCrowd *dust::getCrowd(const string & address)
{
...
bool dust::isPropertyDivisible(unsigned int propertyId)
{
...
string dust::getPropertyName(unsigned int propertyId)
{
  CMPSPInfo::Entry sp;
...
bool dust::isCrowdsaleActive(unsigned int propertyId)
{
...
void dust::dumpCrowdsaleInfo(const string &address, CMPCrowd &crowd, bool bExpired)
{
...
int dust::calculateFractional(unsigned short int propType, unsigned char bonusPerc, uint64_t fundraiserSecs, 
...
bool dust::isCrowdsalePurchase(uint256 txid, string address, int64_t *propertyId, int64_t *userTokens, int64_t *issuerTokens)
{
...
void dust::eraseMaxedCrowdsale(const string &address, uint64_t blockTime, int block)
{
...
unsigned int dust::eraseExpiredCrowdsale(CBlockIndex const * pBlockIndex)
{
...
char *dust::c_strPropertyType(int i)
{
  switch (i)
  {

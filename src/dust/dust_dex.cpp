...
md_PropertiesMap dust::metadex;

md_PricesMap* dust::get_Prices(unsigned int prop)
{
...
md_Set* dust::get_Indexes(md_PricesMap *p, XDOUBLE price)
{
...
void dust::MetaDEx_debug_print(bool bShowPriceLevel, bool bDisplay)
{
  file_log("<<<\n");
...
bool dust::DEx_offerExists(const string &seller_addr, unsigned int prop)
{
...
CMPOffer *dust::DEx_getOffer(const string &seller_addr, unsigned int prop)
{
...
// TODO: locks are needed around map's insert & erase
CMPAccept *dust::DEx_getAccept(const string &seller_addr, unsigned int prop, const string &buyer_addr)
{
...
int dust::DEx_offerCreate(string seller_addr, unsigned int prop, uint64_t nValue, int block, uint64_t amount_des, uint64_t fee, unsigned char btl, const uint256 &txid, uint64_t *nAmended)
{
...
// returns 0 if everything is OK
int dust::DEx_offerDestroy(const string &seller_addr, unsigned int prop)
{
...
// returns 0 if everything is OK
int dust::DEx_offerUpdate(const string &seller_addr, unsigned int prop, uint64_t nValue, int block, uint64_t desired, uint64_t fee, unsigned char btl, const uint256 &txid, uint64_t *nAmended)
{
int rc = DEX_ERROR_SELLOFFER;
...
// returns 0 if everything is OK
int dust::DEx_acceptCreate(const string &buyer, const string &seller, int prop, uint64_t nValue, int block, uint64_t fee_paid, uint64_t *nAmended)
{
...
//
// returns 0 if everything is OK
int dust::DEx_acceptDestroy(const string &buyer, const string &seller, int prop, bool bForceErase)
{
...
int dust::DEx_payment(uint256 txid, unsigned int vout, string seller, string buyer, uint64_t BTC_paid, int blockNow, uint64_t *nAmended)
{
//  if (msc_debug_dex) file_log("%s()\n", __FUNCTION__);
...
prop = DUST_PROPERTY_MSC; //test for MSC accept first
p_accept = DEx_getAccept(seller, prop, buyer);

  if (!p_accept) 
  {
    prop = DUST_PROPERTY_TMSC; //test for TMSC accept second
    p_accept = DEx_getAccept(seller, prop, buyer); 
  }
...
int dust::MetaDEx_ADD(const string &sender_addr, unsigned int prop, uint64_t amount, int block, unsigned int property_desired, uint64_t amount_desired, const uint256 &txid, unsigned int idx)
{
int rc = METADEX_ERROR -1;

  // MetaDEx implementation phase 1 check
  if ((prop != DUST_PROPERTY_MSC) && (property_desired != DUST_PROPERTY_MSC) &&
   (prop != DUST_PROPERTY_TMSC) && (property_desired != DUST_PROPERTY_TMSC))
  {
    return METADEX_ERROR -800;
  }

    // store the data into the temp MetaDEx object here
    CMPMetaDEx new_mdex(sender_addr, block, prop, amount, property_desired, amount_desired, txid, idx, CDustTransaction::ADD);
...
int dust::MetaDEx_CANCEL_AT_PRICE(const uint256 txid, unsigned int block, const string &sender_addr, unsigned int prop, uint64_t amount, unsigned int property_desired, uint64_t amount_desired)
{
int rc = METADEX_ERROR -20;
CMPMetaDEx mdex(sender_addr, 0, prop, amount, property_desired, amount_desired, 0, 0, CDustTransaction::CANCEL_AT_PRICE);
...
int dust::MetaDEx_CANCEL_ALL_FOR_PAIR(const uint256 txid, unsigned int block, const string &sender_addr, unsigned int prop, unsigned int property_desired)
{
...
int dust::MetaDEx_CANCEL_EVERYTHING(const uint256 txid, unsigned int block, const string &sender_addr)
{

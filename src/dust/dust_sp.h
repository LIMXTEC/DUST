...
public:
    ...
    : path(_path)
  {
    ...
    implied_msc.name = "Dust";
    implied_msc.url = "www.dust.org";
    ...
    implied_tmsc.name = "Test Dust";
    implied_tmsc.url = "www.dust.org";
  ...
  void printAll()
  {
    ...
    for (unsigned int idx = DUST_PROPERTY_MSC; idx <= DUST_PROPERTY_TMSC; idx++ ) {
      Entry info;
      printf("%10d => ", idx);
      if (getSP(idx, info)) {
        info.print();

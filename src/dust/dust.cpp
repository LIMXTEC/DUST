...
CMPTxList *dust::p_txlistdb;
CMPTradeList *dust::t_tradelistdb;
CMPSTOList *dust::s_stolistdb;
...
uint32_t dust::GetLatestBlockTime()
{
    LOCK(cs_main);
    if (chainActive.Tip())
...
string dust::strMPProperty(unsigned int i)
{
string str = "*unknown*";
  ...
  switch (i)
  {
    case DUST_PROPERTY_BTC: str = "BTC"; break;
    case DUST_PROPERTY_MSC: str = "MSC"; break;
    case DUST_PROPERTY_TMSC: str = "TMSC"; break;
...
std::string dust::FormatIndivisibleMP(int64_t n)
{
...
OfferMap dust::my_offers;
AcceptMap dust::my_accepts;

CMPSPInfo *dust::_my_sps;
CrowdMap dust::my_crowds;

PendingMap dust::my_pending;
...
std::map<string, CMPTally> dust::mp_tally_map;

CMPTally *dust::getTally(const string & address)
{
  LOCK (cs_tally);
...
bool dust::isTestEcosystemProperty(unsigned int property)
{
  if ((DUST_PROPERTY_TMSC == property) || (TEST_ECO_PROPERTY_1 <= property)) return true;

  return false;
}

bool dust::isMainEcosystemProperty(unsigned int property)
{
  if ((DUST_PROPERTY_BTC != property) && !isTestEcosystemProperty(property)) return true;

  return false;
}

bool dust::isMetaDExOfferActive(const uint256 txid, unsigned int propertyId)
{
...
std::string dust::getDustAlertString()
{
    return global_alert_message;
}

std::string dust::getTokenLabel(unsigned int propertyId)
{
    std::string tokenStr;
    if (propertyId < 3) {
...
bool dust::parseAlertMessage(std::string rawAlertStr, int32_t *alertType, uint64_t *expiryValue, uint32_t *typeCheck, uint32_t *verCheck, std::string *alertMessage)
{
...
std::string dust::getDustAlertTextOnly()
{
    std::vector<std::string> vstr;
...
bool dust::checkExpiredAlerts(unsigned int curBlock, uint64_t curTime)
{
        ...
        if (parseSuccess)
        {
            switch (alertType)
            {
                 ...
                 case 4: //Update alert, show upgrade alert in UI and getalert_MP call + use isTransactionTypeAllowed to verify client support and shutdown if not present
                     ...
                     bool txSupported = isTransactionTypeAllowed(expiryValue+1, DUST_PROPERTY_MSC, typeCheck, verCheck);
                     ...
                     if ((!txSupported) && (txLive))
                     {
                         ...
                         if (!GetBoolArg("-overrideforcedshutdown", false)) AbortNode("Shutting down due to alert: " + getDustAlertTextOnly());
                         return false;
                     }
...
int64_t dust::getTotalTokens(unsigned int propertyId, int64_t *n_owners_total)
{
...
bool dust::update_tally_map(string who, unsigned int which_property, int64_t amount, TallyType ttype)
{
bool bRet = false;
...
bool dust::isTransactionTypeAllowed(int txBlock, unsigned int txProperty, unsigned int txType, unsigned short version, bool bAllowNullProperty)
{
bool bAllowed = false;
...
unsigned short version_TopAllowed;
  ...
  if ((DUST_PROPERTY_BTC == txProperty) && !bAllowNullProperty) return false;
...
const double available_reward=all_reward * part_available;
  ...
  update_tally_map(exodus_address, DUST_PROPERTY_MSC, exodus_delta, BALANCE);
  exodus_prev = devmsc;
...
vector<unsigned char> vec_chars;
  ...
  // do only as many re-hashes as there are dust packets, 255 per spec
...
int TXExodusFundraiser(const CTransaction &wtx, const string &sender, int64_t ExodusHighestValue, int nBlock, unsigned int nTime)
{
  ...
  { //Exodus Fundraiser start/end blocks
    ...
    update_tally_map(sender, DUST_PROPERTY_MSC, msc_tot, BALANCE);
    update_tally_map(sender, DUST_PROPERTY_TMSC, msc_tot, BALANCE);
...
int parseTransaction(bool bRPConly, const CTransaction &wtx, int nBlock, unsigned int idx, CDustTransaction *mp_tx, unsigned int nTime)
{
string strSender;
...
uint64_t txFee = 0;
            ...
            string strAddress;
              ...
              if (ExtractDestination(wtx.vout[i].scriptPubKey, dest))
              {
                ...
                if ((exodus_address != strAddress) && (validType))
                {
                  ...
                  //wtx.vout[i].scriptPubKey.mscore_parse(script_data); // TODO: step1
            ...
            // go through inputs, find the largest per Dust protocol, the Sender
...
//              file_log("%s size=%d (%s); ", address.ToString().c_str(), keyID.size(), keyID.GetHex().c_str());
          ...
          //wtx.vout[i].scriptPubKey.mscore_parse(multisig_script_data, false); // TODO: step1
        }
              }
...
// parse blocks, potential right from Dust initial
...
const int max_block = GetHeight();
  ...
  for (int blockNum = nHeight;blockNum<=max_block;blockNum++)
  {
    ...
    int tx_count = 0;
    dust_handler_block_begin(blockNum, pblockindex);
    BOOST_FOREACH(const CTransaction&tx, block.vtx)
    {
      if (0 == dust_handler_tx(tx, blockNum, tx_count, pblockindex)) n_found++;
    ...
    dust_handler_block_end(blockNum, pblockindex, n_found);
  }
...
int input_msc_balances_string(const string &s)
{
  ...
  for (iter = vProperties.begin(); iter != vProperties.end(); ++iter) {
    ...
    int property = DUST_PROPERTY_MSC;
...
int input_mp_offers_string(const string &s)
{
  ...
  if (DUST_PROPERTY_BTC == prop_desired)
  {
...
{
  unsigned int nextSPID = _my_sps->peekNextSPID(DUST_PROPERTY_MSC);
  unsigned int nextTestSPID = _my_sps->peekNextSPID(DUST_PROPERTY_TMSC);
...
int dust_init()
{
  ...
  file_log("\n%s DUST INIT, build date: " __DATE__ " " __TIME__ "\n\n", DateTimeStrFormat("%Y-%m-%d %H:%M:%S", GetTime()).c_str());
...
//    if (isNonMainNet()) nWaterlineBlock = 272700;
  ...
  exodus_balance = getMPbalance(exodus_address, DUST_PROPERTY_MSC, BALANCE);
...
int dust_shutdown()
{
  ...
  file_log("\n%s DUST SHUTDOWN, build date: " __DATE__ " " __TIME__ "\n\n", DateTimeStrFormat("%Y-%m-%d %H:%M:%S", GetTime()).c_str());
...
int dust::ClassB_send(const string &senderAddress, const string &receiverAddress, const string &redemptionAddress, const vector<unsigned char> &data, uint256 & txid, int64_t referenceamount)
{
CWallet *wallet = pwalletMain;
CCoinControl coinControl;
...
uint256 dust::send_INTERNAL_1packet(const string &FromAddress, const string &ToAddress, const string &RedeemAddress, unsigned int PropertyID, uint64_t Amount, unsigned int PropertyID_2, uint64_t Amount_2, unsigned int TransactionType, int64_t additional, int *error_code)
{
...
const unsigned int prop = PropertyID;
  ...
  if((TransactionType == MSC_TYPE_METADEX) && ((additional == CDustTransaction::CANCEL_AT_PRICE) || (additional == CDustTransaction::CANCEL_ALL_FOR_PAIR) || (additional == CDustTransaction::CANCEL_EVERYTHING)))
  {
...
int CMPTxList::setLastAlert(int blockHeight)
{
    ...
    for(it->SeekToFirst(); it->Valid(); it->Next())
    {
       ...
       if (4 == vstr.size())
       {
           if (atoi(vstr[2]) == DUST_MESSAGE_TYPE_ALERT) // is it an alert?
           {
...
void CMPTxList::recordMetaDExCancelTX(const uint256 &txidDust, const uint256 &txidSub, bool fValid, int nBlock, unsigned int propertyId, uint64_t nValue)
{
  if (!pdb) return;
       ...
       string txidDustStr = txidDust.ToString() + "-C";
       ...
       std::vector<std::string> vstr;
       string strValue;
       Status status = pdb->Get(readoptions, txidDustStr, &strValue);
       if (status.ok())
       {
           // parse the string returned
           boost::split(vstr, strValue, boost::is_any_of(":"), token_compress_on);
       ...
       const string txidStr = txidDust.ToString() + "-C";
...
// figure out if there was at least 1 Dust Protocol transaction within the block range, or a block if starting equals ending
...
bool dust::isMPinBlockRange(int starting_block, int ending_block, bool bDeleteFound)
{
...
//
bool dust::getValidMPTX(const uint256 &txid, int *block, unsigned int *type, uint64_t *nAmended)
{
string result;
int validity = 0;
...
int dust_handler_block_begin(int nBlockPrev, CBlockIndex const * pBlockIndex) {
...
int dust_handler_block_end(int nBlockNow, CBlockIndex const * pBlockIndex,
...
// 2) update the amount in the Exodus address
  ...
  if (msc_debug_exo)
    file_log("devmsc for block %d: %lu, Exodus balance: %lu\n", nBlockNow,
        devmsc, getMPbalance(exodus_address, DUST_PROPERTY_MSC, BALANCE));
  ...
  // force an update of the UI once per processed block containing DUST transactions
  if (countMP > 0)  // there were DUST transactions in this block
  {
    //uiInterface.DustStateChanged(); // TODO: step1 important
  }

  // save out the state after this block
  if (writePersistence(nBlockNow))
    dust_save_state(pBlockIndex);
...
static const string addr = "1MpNote1jsHkbQLwEmgoMr29EoUC1nyxxV";
 ...
 //
int CDustTransaction::interpretPacket(CMPOffer *obj_o, CMPMetaDEx *mdex_o)
{
int rc = PKT_ERROR;
int step_rc;
...
int CDustTransaction::logicMath_SimpleSend()
{
...
int CDustTransaction::logicMath_SendToOwners(FILE *fhandle)
{
int rc = PKT_ERROR_STO -1000;
      ...
      const unsigned int feeProperty = isTestEcosystemProperty(property) ? DUST_PROPERTY_TMSC : DUST_PROPERTY_MSC;
